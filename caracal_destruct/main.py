...
@click.option(
    ...
    "--bands",
    type=str,
    help="CASA-style comma separated bands (or spws) to parallize the pipeline over. Example, "
...
def driver(config_file, spw_split_spec, bands, batchconfig, skip, singularity_image_dir, boring, dryrun, log_level):
    ...
    runit = SlurmRun(config_file, batchdict, skiplist=skiplist, bands=bands, spwid=sss["spwid"], nband=sss["nband"],
                    nchan=["nchan"], singularity_image_dir=singularity_image_dir, dryrun=dryrun)
    runit.submit()
