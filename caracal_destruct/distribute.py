...
class MSRun:
    def __init__(self, ms: str=None, band:str = None, label:str = None, prefix:str = None, 
                options: Dict[str, DestructValueType] = None, imports: List[str] = None):
        self.ms = ms
        self._band = band
        self.label = label
        self._prefix = prefix
    ...
    def set_prefixes(self, pipeline, bands=None):
        if bands:
            if len(bands) != self.nruns:
                raise DistributionException("Number of requested bands does not match number of runs in config.caraca.runs")
            else:
                set_bands = True
        else:
            set_bands = False
...
class Scatter():
    ...
    def set(self, nband=None, bands=None):
        ...
        self.runs = []
        for i, msrun in enumerate(self.config.runs):
            thisrun = {}
